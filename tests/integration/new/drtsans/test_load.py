...
class TestLoadEvents():
    ...
    def test_pixel_calibration(self, reference_dir):
        ...
        workspace = load_events(file_name,
        ...
        assert nominal_pixel_width == pytest.approx(0.00804, abs=1.e-5)  # uncalibrated width
        pixel_width = component_info.scaleFactor(42).X() * nominal_pixel_width
        assert pixel_width == pytest.approx(0.00968, abs=1.e-5)  # calibrated width

        # Assert pixel height is calibrated
        nominal_pixel_height = component_info.shape(component_info_index).getBoundingBox().width().Y()
        assert nominal_pixel_height == pytest.approx(0.00409, abs=1.e-5)  # uncalibrated width
        pixel_height = component_info.scaleFactor(42).X() * nominal_pixel_height
        assert pixel_height == pytest.approx(0.00492, abs=1.e-5)  # calibrated width
