...
def test_half_polarization():
    ...
    efficiency = CreateSingleValuedWorkspace(DataValue=0.998, ErrorValue=0.001,
                                             OutputWorkspace=uwd())

    # values for the measured flipper off (M0) and flipper on (M1)
    M0 = CreateSingleValuedWorkspace(DataValue=10000, ErrorValue=100,
                                     OutputWorkspace=uwd())
    M1 = CreateSingleValuedWorkspace(DataValue=8100, ErrorValue=90,
                                     OutputWorkspace=uwd())

    # expected results
    SpinUpExp = CreateSingleValuedWorkspace(DataValue=10050.100, ErrorValue=103.2046,  # was 103.205
                                            OutputWorkspace=uwd())
    SpinDownExp = CreateSingleValuedWorkspace(DataValue=8046.0925, ErrorValue=93.2163,
                                              OutputWorkspace=uwd())
