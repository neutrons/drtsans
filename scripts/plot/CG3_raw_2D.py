...
def load_data(filename):
    ...
    data1 = np.rot90(np.array(root.find("Data").findtext("Detector").split(),
                              dtype=int).reshape((192, 256)))
    data2 = np.rot90(np.array(root.find("Data").findtext("DetectorWing")
                              .split(), dtype=int).reshape((160, 256)))
...
plot = ax1.imshow(data1, norm=LogNorm(), extent=(0.5, 192.5, 0.5, 256.5),
                  origin='lower')
plot = ax2.imshow(data2, norm=LogNorm(), extent=(0.5, 160.5, 0.5, 256.5),
                  origin='lower')
