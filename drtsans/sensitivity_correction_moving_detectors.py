...
def _calculate_weighted_average_with_error(normalized_data, normalized_error):
    ...
    avg_norm_error = (
        normalized_data
        / weighted_average
        * np.sqrt((normalized_error / normalized_data) ** 2 + (weighted_average_error / weighted_average) ** 2)
    )
...
def _calculate_pixel_wise_sensitivity(flood_data, flood_error):
    ...
    s_ij = np.nansum(1.0 / flood_error**2, axis=0)  # summation along axis 1: among files
...
def _normalize_sensitivities(d_array, sigam_d_array):
    ...
    sensitivities_error = (
        d_array / sens_avg * np.sqrt((sigam_d_array / d_array) ** 2 + (sigma_sens_avg / sens_avg) ** 2)
    )
