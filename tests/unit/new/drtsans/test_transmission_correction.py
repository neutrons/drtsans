...
@pytest.mark.parametrize('workspace_with_instrument',
                         [dict(name='EQSANS', Nx=10, Ny=10, dx=1e-3, dy=1e-3, zc=0.01)],
                         indirect = True)
...
@pytest.mark.parametrize('generic_workspace',
                         [{'name': 'I_sam',
                           ...
                           'intensities': I_sam}],
                         indirect = True)
def test_transmission_correction(generic_workspace, workspace_with_instrument):
    '''
    ...
    #print the drtsans result
    print(result.extractY())

    #print the by-hand calculation
    print(I_tsam)

    np.testing.assert_equal(result.extractY(), I_tsam)
