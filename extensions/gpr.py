...
def autoreduction_plots(reduction_output, output_dir: str, output_filename: str, conf: dict) -> str:
    """Generate GPR analysis plots for EQSANS autoreduction.

    This is the main integration point for autoreduction scripts. It processes
    reduction output and generates GPR plots for I(Q) profiles, handling multiple
    configurations (wedges, time slices, etc.).
    ...
    - Only runs if GPR is enabled in configuration
    - This function replaces the previous generate_gpr_plots in reduce_EQSANS.py

    Examples
    --------
    >>> from drtsans.extensions.gpr import autoreduction_plots
    >>> html = autoreduction_plots(reduction_output, "/output", "EQSANS_172839", config)
    """
    # Check configuration for GPR enable/disable flag
    enable_gpr = conf.get("enableGPR", True)  # default to True if not specified
    if not enable_gpr:
        logger.information("GPR analysis skipped: disabled in configuration")
        return ""

    logger.information("Starting GPR analysis for I(Q) profiles")
    report = ""
    ...
    try:
        ...
        for i, profile_set in enumerate(reduction_output):
            ...
            if conf["1DQbinType"] == "wedge" and len(profiles) > 1:
                ...
                for j, profile in enumerate(profiles):
                    ...
                    logger.information(f"Running GPR for profile set {i}, wedge {j}")
                    ...
                    try:
                        ...
                        if html:
                            report += html
                        logger.information(
                            f"GPR complete for {base_name}: {len(png_files)} PNG, {len(dat_files)} DAT files"
                        )
                    except Exception as e:
                ...
                # Single profile or list of profiles (scalar or annular binning)
                base_name = f"{output_filename}{slice_suffix}"
                logger.information(f"Running GPR for profile set {i}")

                try:
                    # generate_gpr_analysis handles both single IQmod and list of IQmod
                    html, png_files, dat_files = generate_gpr_analysis(profiles, output_dir, base_name)
                    if html:
                        report += html
                    logger.information(
                        f"GPR complete for {base_name}: {len(png_files)} PNG, {len(dat_files)} DAT files"
        ...
        if report:
            logger.information("GPR analysis completed successfully")
        else:
