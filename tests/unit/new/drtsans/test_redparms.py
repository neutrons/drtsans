...
def redparms_data():
    ...
    schema_instrument = json.loads(r"""
    {
      "$schema": "http://json-schema.org/draft-07/schema#",
      "type": "object",
      "properties": {
    ...
    reduction_parameters = json.loads(r"""
    ...
    yield {'schema_common': schema_common,
...
class TestReferenceResolver:
    ...
    def test_resolve_uri(self, redparms_data):
        ...
        compared = {"anyOf": [{"type": "string", "minLength": 1, "pattern": "^[1-9][0-9]*$"},
                              {"type": "integer", "minimum": 1}],
                    "preferredType": "int", "description": "The IPTS number for the data files", "examples": ["24769"]}
        assert resolved == compared
        resolved = resolver._resolve_uri("common.json#/configuration/timeSliceInterval")
        compared = {"$ref": "common.json#/definitions/safeStringPositiveFloat",
                    "description": "Interval for time slicing"}
        assert resolved == compared

    def test_derefence(self, redparms_data):
        resolver = ReferenceResolver(redparms_data['schema_common_file'])
        unresolved = {"background": {"iptsNumber": {"$ref": "common.json#/iptsNumber"}, "default": 12345}}
        ...
        compared = {'configuration': {
            'outputDir': {'type': 'string', 'description': 'Output folder'},
            'timeSliceInterval': {
                'description': 'Interval for time slicing', 'preferredType': 'float',
                'anyOf': [{'type': 'string', 'pattern': '^$|^[0-9]*.[0-9]*$'},
                          {'type': 'number', 'exclusiveMinimum': 0}]}}}
        assert resolved == compared
        unresolved = {"$schema": "http://json-schema.org/draft-07/schema#", "type": "object",
                      "configuration": {
                          "type": "object",
                          "properties": {
                              "outputDir": {"$ref": "common.json#/configuration/outputDir"}
        ...
        resolved = resolver.dereference(unresolved)
        compared = {'$schema': 'http://json-schema.org/draft-07/schema#', 'type': 'object',
                    'configuration': {
                        'type': 'object',
                        'properties': {
                            'outputDir': {'type': 'string', 'description': 'Output folder'}
        ...
        compared = {'$schema': 'http://json-schema.org/draft-07/schema#', 'type': 'object',
                    'properties': {
                        'sample': {
                            'type': 'object',
                            'properties': {
                                'runNumber': {'description': 'The run number(s) for the sample',
                                              'anyOf': [{'type': 'string', 'minLength': 1, 'pattern': '^[1-9][0-9]*$'},
                                                        {'type': 'integer', 'minimum': 1},
                                                        {'type': 'array', 'items': {'type': ['integer', 'string']}}],
                                              'preferredType': 'int'},
                                    ...
                                    'properties': {
                                        'runNumber': {
                                            ...
                                            'anyOf': [{'type': 'string', 'minLength': 1, 'pattern': '^[1-9][0-9]*$'},
                                                      ...
                                                      {'type': 'array', 'items': {'type': ['integer', 'string']}}],
                                            'preferredType': 'int'},
                                        'value': {'anyOf': [{'type': 'string', 'pattern': '^$|^0?.[0-9]*$'},
                                                            {'type': 'number', 'exclusiveMinimum': 0, 'maximum': 1}],
                                                  'preferredType': 'float'}},
                                    'maxProperties': 2, 'required': ['runNumber', 'value'],
                                    'description': 'The transmission for the sample'}},
                            'maxProperties': 2, 'required': ['transmission', 'runNumber']}}}
        assert resolved == compared
        unresolved = {"$schema": "http://json-schema.org/draft-07/schema#", "type": "object",
                      "properties": {
                          "sample": {"$ref": "common.json#/sample"},
                      ...
                      "maxProperties": 2, "required": ["sample", "configuration"]}
        resolved = resolver.dereference(unresolved)
        compared = {'$schema': 'http://json-schema.org/draft-07/schema#', 'type': 'object',
                    'properties': {
                        'sample': {
                        ...
                        'configuration': {
                            'type': 'object',
                            'properties': {
                                'timeSliceInterval': {'description': 'Interval for time slicing',
                                                      'anyOf': [{'type': 'string', 'pattern': '^$|^[0-9]*.[0-9]*$'},
                                                                {'type': 'number', 'exclusiveMinimum': 0}],
                                                      'preferredType': 'float'}}}},
                    'maxProperties': 2, 'required': ['sample', 'configuration']}
...
class TestSuggest:
    ...
    property_names = ['instrumentName', 'iptsNumber', 'sample', 'thickness', 'transmission', 'runNumber', 'runNumber']

    @pytest.mark.parametrize('query, score', [('home', 0), ('Home', 1), ('hom', 1), ('ho_me', 1), ('Ho_E', 3)])
...
class TestDefaultJson:
    ...
    def test_trim_schema(self, default_json):
        compared = {'instrumentName': 'BIOSANS', 'iptsNumber': 42,
                    'sample': {'runNumber': None, 'transmission': {'runNumber': None, 'value': None}},
