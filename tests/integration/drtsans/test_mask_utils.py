...
def test_apply_mask(clean_workspace):
    w = LoadEmptyInstrument(InstrumentName="EQ-SANS", OutputWorkspace=clean_workspace(uwd()))
    #
    # using MaskBPT with a Component
    #
    apply_mask(w, Components="back-panel")
    m = ExtractMask(w, OutputWorkspace=clean_workspace(uwd())).OutputWorkspace
    assert isinstance(m, MaskWorkspace)
    ClearMaskFlag(w)
    MaskBTP(Workspace=w, Components="back-panel")
    m2 = ExtractMask(w, OutputWorkspace=clean_workspace(uwd())).OutputWorkspace
    ...
    m2 = ExtractMask(w, OutputWorkspace=clean_workspace(m2.name())).OutputWorkspace
    assert CompareWorkspaces(m, m2).Result
    #
    ...
    ClearMaskFlag(w)
    MaskBTP(Workspace=w, Bank="1-24", Pixel="1-10")
    m = ExtractMask(w, OutputWorkspace=clean_workspace(m.name())).OutputWorkspace
