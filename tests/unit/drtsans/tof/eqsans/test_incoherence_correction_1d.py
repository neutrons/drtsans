...
def test_calculate_b_factor():
    ...
    test_b_array = calculate_b_error_b(
        wl_vec, i_array, error_array, qmin_index, qmax_index, ref_wl_ie, True, intensity_weighted=True
    )
    ...
    test_b_array = calculate_b_error_b(wl_vec, i_array, error_array, 4, 9, ref_wl_ie, True, intensity_weighted=True)
...
def test_incoherence_inelastic_correction(temp_directory):
    ...
    np.testing.assert_allclose(corrected_i_array, generate_expected_corrected_intensities(), verbose=True)
        ...
        for q_j in range(len(q_vec)):
            if qmin_index <= q_j <= qmax_index:
                ...
                for q_k in range(qmin_index, qmax_index + 1):
                    ...
                    error2_j += 1.0 / n_q**2 * (ref_wl_ie.error_vec[q_k] ** 2 + error_array[q_k, i_wl] ** 2)
            else:
...
def calculate_b_vector_prototype(wl_vec, i_array, error_array, qmin_index, qmax_index, ref_wl_ie, calculate_delta_b):
    ...
    for i_wl in range(len(wl_vec)):
        ...
        for j_q in range(qmin_index, qmax_index + 1):
            ...
            if calculate_delta_b:
                ...
                delta_b_i += 1.0 / n_q**2 * (ref_wl_ie.error_vec[j_q] ** 2 + error_array[j_q, i_wl] ** 2)
