...
def simple_tubes_panel(workspace_with_instrument):
    workspace = workspace_with_instrument(axis_values=[1.0, 2.0],
                                          intensities=np.arange(9).reshape((9, 1)))
...
def test_resolve_indexes(simple_tubes_panel):
    ...
    assert tubecollection._resolve_indexes(workspace, 4, None) == (4, 4)  # resolve the workspace index
    assert tubecollection._resolve_indexes(workspace, None, 4) == (4, 4)  # resolve the component info index
