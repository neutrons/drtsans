...
def test_translated_gpsans(reference_dir):
    ...
    # Load data
    workspace = LoadEventNexus(Filename=test_nexus_file,
                               OutputWorkspace=unique_workspace_dundername(),
                               LoadLogs=True)
    assert workspace

    # Verify calculated SDD against SDD in meta data
    calculated_sdd = sample_detector_distance(workspace, unit='m', log_key=None, search_logs=False)
    das_sdd = sample_detector_distance(workspace, unit='m', log_key=None, search_logs=True, forbid_calculation=True)
    assert abs(calculated_sdd - das_sdd) == pytest.approx(0, rel=1E-7), f'{das_sdd} vs {calculated_sdd}'
    ...
    assert abs(calculated_sdd - expected_sdd) == pytest.approx(0, abs=1E-7), f'{calculated_sdd}, {expected_sdd}'
