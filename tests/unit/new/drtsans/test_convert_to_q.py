...
def test_convert_to_mod_q(generic_workspace):
    ...
    # the following will fail at 1e-5 since the average of q(lam) is not the q(average lam)
    q = 4. * np.pi * np.sin(two_theta * 0.5) / 6.
    assert modq == approx([q, q, q], abs=1e-4)
...
def test_convert_q_azimuthal(generic_workspace):
    ...
    two_theta = np.arccos(5./np.sqrt(25.5))
    q = 4. * np.pi * np.sin(two_theta * 0.5) / 6.
    assert result.qx == approx([q * np.sqrt(0.5), q*np.sqrt(0.5), -q*np.sqrt(0.5)], abs=1e-4)
    assert result.qy == approx([-q * np.sqrt(0.5), q*np.sqrt(0.5), q*np.sqrt(0.5)], abs=1e-4)

    # test with simple resolution
    result = convert_to_q(ws, mode='azimuthal', resolution_function=fake_resolution1)
    assert result.delta_qx == approx([0, q * np.sqrt(2.), 0], abs=1e-4)
    assert result.delta_qy == approx([q * np.sqrt(2.), 0, -q * np.sqrt(2.)], abs=1e-4)
