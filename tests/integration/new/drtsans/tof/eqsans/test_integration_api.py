...
values = (('EQSANS_86217', 508339, 1300, 14122, 9595, 59633, True,
           2.61, 14.72, 540),
          ('EQSANS_92353', 262291, 4000, 14122, 11288, 61309, True,
           2.59, 12.98, 431),
          ('EQSANS_85550', 270022, 5000, 14122, 11914, 61930, True,
           2.59, 12.43, 599),
          ('EQSANS_101595', 289989, 1300, 14122, 7649, 24392, False,
           2.11, 5.65, 141),
          ('EQSANS_88565', 19362, 4000, 14122, 45485, 62174, False,
           10.02, 13.2, 694),
          ('EQSANS_88901', 340431, 8000, 14122, 66629, 83378, False,
           11.99, 14.62, 54377))
...
class TestLoadEvents(object):
    ...
    def test_geometry(self, run_infoset):
        ...
        # Check logs
        sl = SampleLogs(ws)
        assert run_infoset.ssd == approx(sl.single_value('source-sample-distance'),
                                         abs=1)
        assert run_infoset.sdd == approx(sl.single_value('sample-detector-distance'),
                                         abs=1)
...
def test_normalize_by_flux(run_infoset, flux_file):
    ...
    normalized_data_workspace = eqsans.normalize_by_flux(data_workspace, flux_file, method='proton charge',
                                                         output_workspace=unique_workspace_dundername())
    normalized_data_workspace = SumSpectra(normalized_data_workspace)
    assert 1.e6 * np.average(normalized_data_workspace.readY(0)) == approx(run_infoset.flux_normalized, abs=1.)
...
def test_prepare_monitors(reference_dir):
    ...
    with amend_config(data_dir=data_dirs):
        ...
        v = LoadNexus('EQSANS_88565_monitors_wav.nxs', OutputWorkspace=unique_workspace_dundername())
...
def test_solid_angle(run_infoset):
    ws2 = solid_angle_correction(run_infoset.ws, output_workspace=unique_workspace_dundername(),
