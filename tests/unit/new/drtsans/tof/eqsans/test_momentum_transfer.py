...
expected_qx = np.array([[[-0.006134, -0.003254, -0.000374, 0.002505, 0.005385],
...
expected_qy = np.array([[[0.004962, 0.004962, 0.004962, 0.004962, 0.004962],
...
expected_dqx = np.array([[[0.000066, 0.000035, 0.000005, 0.000027, 0.000058],
...
expected_dqy = np.array([[[0.000054, 0.000054, 0.000054, 0.000054, 0.000054],
...
@pytest.mark.parametrize('workspace_with_instrument',
                         [{'Nx': 5, 'Ny': 5,
                           'name': 'EQSANS',
                           'dx': 0.0055, 'dy': 0.00425,
                           'xc': 0.000715, 'yc': 0.0009775, 'zc': 4., 'l1': 4.}],
                         indirect=True)
def test_convert_to_q_eqsans(workspace_with_instrument):
    ws = workspace_with_instrument(axis_values=[2.95, 3.05, 3.15, 3.25],
                                   intensities=intensities_array,
                                   uncertainties=np.sqrt(intensities_array), view='array')
    AddSampleLog(Workspace=ws, LogName='source-aperture-diameter', LogText='50.', LogUnit='mm', LogType='Number')
    AddSampleLog(Workspace=ws, LogName='sample-aperture-diameter', LogText='20.', LogUnit='mm', LogType='Number')
