...
def createIQAzimuthal(with_resolution=True):
    ...
    resolutionArgs = {}
    if with_resolution:
        resolutionArgs['delta_qx'] = np.full((21, 21), 1., dtype=float)
        resolutionArgs['delta_qy'] = np.full((21, 21), 1., dtype=float)

    return IQazimuthal(intensity=np.arange(21*21, dtype=float).reshape((21, 21)),
