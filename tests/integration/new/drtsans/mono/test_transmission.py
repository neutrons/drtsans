...
@pytest.mark.parametrize('workspace_with_instrument',
                         [dict(name='MYSANS', Nx=5, Ny=1, dx=0.1, dy=0.01, zc=1.3)], indirect=True)
def test_calculate_theta_dependent_transmission_single_value(workspace_with_instrument):
    ...
    sample_counts = np.ones(n_pixels).reshape((n_pixels, 1))  # each pixel has an intensity of one
    sample_errors = np.zeros(n_pixels).reshape((n_pixels, 1))  # only interested in errors from transmission
    ...
    MoveInstrumentComponent(sample_workspace, ComponentName='detector1', X=0.20, RelativePosition=True)
    ...
    assert two_thetas[::-1] == approx([np.arctan(x * 10 / 130) for x in range(5)], abs=0.001)

    # Apply the transmission correction to our sample intensities
    sample_workspace = apply_transmission_correction(sample_workspace, trans_value=0.9, trans_error=0.1,
    ...
    assert transmissions.extractY().flatten()[::-1] == approx(test17a_transmissions, abs=0.0001)
    ...
    assert transmissions_sigma_square == pytest.approx(test17a_transmissions_sigma_square, abs=0.0001)
