...
import ornl.sans.hfir.biosans.beam_finder as biosans_bf
import ornl.sans.hfir.gpsans.beam_finder as gpsans_bf
import ornl.sans.sns.eqsans.beam_finder as eqsans_bf
...
def test_beam_finder_trivial(generic_IDF):
    ...
    Functions to test: ornl.sans.sns.eqsans.beam_finder.find_beam_center
    ...
    assert inst.getDetector(5).getPos() == approx([1, 1, 5], abs=1e-5)
    assert inst.getDetector(6).getPos() == approx([1, 2, 5], abs=1e-5)
    assert inst.getDetector(9).getPos() == approx([2, 1, 5], abs=1e-5)
    assert inst.getDetector(10).getPos() == approx([2, 2, 5], abs=1e-5)
    x_bio, y_bio, _ = biosans_bf.find_beam_center(ws)
    assert x_bio == approx(1.692308, abs=1e-5)
    assert y_bio == approx(1.384615, abs=1e-5)
    x_gp, y_gp = gpsans_bf.find_beam_center(ws)
    assert x_gp == approx(1.692308, abs=1e-5)
    assert y_gp == approx(1.384615, abs=1e-5)
    x_eqs, y_eqs = eqsans_bf.find_beam_center(ws)
    assert x_eqs == approx(1.692308, abs=1e-5)
...
def test_beam_finder_larger_workspace(generic_workspace):
    ...
    # test masking
    inst = ws.getInstrument()
    assert inst.getDetector(mask[0]). getPos() == approx([8, 2, 5], abs=1e-5)
    assert inst.getDetector(mask[1]). getPos() == approx([8, 3, 5], abs=1e-5)
    assert inst.getDetector(mask[2]). getPos() == approx([9, 2, 5], abs=1e-5)
    assert inst.getDetector(mask[3]). getPos() == approx([9, 3, 5], abs=1e-5)
    spec = ws.spectrumInfo()
    for i in mask:
        assert spec.isMasked(i)
    # test functions
    x_bio, y_bio, _ = biosans_bf.find_beam_center(ws)
    assert x_bio == approx(5.576087, abs=1e-5)
    assert y_bio == approx(5.654682, abs=1e-5)
    x_eqs, y_eqs = eqsans_bf.find_beam_center(ws)
    assert x_eqs == approx(5.576087, abs=1e-5)
    assert y_eqs == approx(5.654682, abs=1e-5)
    x_gp, y_gp = gpsans_bf.find_beam_center(ws)
    assert x_gp == approx(5.576087, abs=1e-5)
