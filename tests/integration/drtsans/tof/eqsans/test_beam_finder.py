...
def test_find_beam_center(datarepo_dir, eqsans_p, temp_workspace_name):
    ...
    with NamedTemporaryFile(delete=True, suffix=".xml") as f:
        ...
        assert xy[:2] == approx((x0, y0), abs=1e-04)
    #
    ...
    assert find_beam_center(ws)[:2] == pytest.approx((0, 0), abs=1e-04)
