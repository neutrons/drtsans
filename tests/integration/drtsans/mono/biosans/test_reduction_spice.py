...
def test_spice_reduction(datarepo_dir, temp_directory):
    ...
    dark_mfname = (44, 1)
    dark_wfname = (44, 1)
    sens_mfname = os.path.join(datarepo_dir.biosans, "Sens_f6368m4p0_bsSVP.nxs")
    sens_wfname = os.path.join(datarepo_dir.biosans, "Sens_f6380w1p4_bsSVP.nxs")
    ...
    base_output_directory = temp_directory("cg3_reduction_test")

    scaling_beam_radius = None
    flexible_pixelsizes = False  # 'True'- if use barscan/flood information for flexible pixel sizes, else 'False'
    ...
    test_log_files = reduce_biosans_nexus(
        ...
        OL_range_wedge1,
        refreshCycle,
        nexus_dir=os.path.join(datarepo_dir.biosans, "Exp318"),
    )
