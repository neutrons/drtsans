...
data_source_aperture = [DataSourceAperture(9999, 1, 7, 4, 0.005, 4.042),
...
def test_source_aperture(generic_workspace, data):
    ...
    assert result.diameter == pytest.approx(data.diameter, abs=1.e-05)
    assert result.distance_to_sample == pytest.approx(data.asd, abs=1.e-05)
