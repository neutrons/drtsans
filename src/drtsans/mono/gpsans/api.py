...
def load_all_files(
    ...
    # Adjust pixel positions, pixel heights and widths
    ...
    if timeslice or logslice or polarized:
        ...
        if not registered_workspace(ws_name):
            ...
            logger.information(
                "[META] Wavelength range is from {} to {}".format(mtd[ws_name].x(0)[0], mtd[ws_name].x(0)[1])
            )
            # Apply mask
            for btp_params in default_mask:
...
def form_output_name(workspace):
    ...
    file_name = workspace_name.rsplit("/", maxsplit=1)[-1].split(".", maxsplit=1)[0]
